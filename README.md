...
LT、勉強会、発表会などで、参加者が Discord チャンネルに投稿したコメントを発表者のブラウザ画面に重ねて表示できます。Chrome 拡張機能が制限されていないサイトであれば、通常の `http/https` ページで動作します。
...
3. ポップアップの `接続` を ON にする。
4. 画面右下、またはポップアップに表示される合言葉を確認する。
5. ポップアップの `コピー` で `/lt start <合言葉>` をコピーする。
6. Discord の対象チャンネルにコピーしたコマンドを貼り付けて実行する。
7. 以後、そのチャンネルへの投稿が画面上に流れる。
